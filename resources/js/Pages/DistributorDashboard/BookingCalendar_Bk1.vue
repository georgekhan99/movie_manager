...
// ✅ Fix placement and duration association to ensure correct grouping
        ...
        <table class="min-w-full border-collapse border border-gray-300">
          <thead>
            <tr class="bg-gray-200 text-gray-700">
              <th class="border border-gray-300 px-6 py-3 w-1/5">Cinema</th>
              <th class="border border-gray-300 px-6 py-3 w-1/5">Placement</th>
              <th v-for="(duration, index) in durations" :key="duration.id" class="border border-gray-300 px-6 py-3 text-center w-1/10">
                {{ getDateRange(duration.start_date) }}
              </th>
            </tr>
          </thead>
          ...
          <tbody>
            <template v-for="(cinema, index) in getPlacementDurations" :key="index">
              <template v-for="(placement, idx) in cinema.placements" :key="placement.placement_id">
                <tr>
                  <!-- Render cinema name only in the first row -->
                  <td v-if="idx === 0" :rowspan="cinema.placements.length" class="border border-gray-300 px-6 py-4 font-medium text-gray-900 align-top">
                    {{ cinema.cinema_name }}
                  </td>
                  ...
                  <td v-for="(duration, i) in durations" :key="duration.id" class="border border-gray-300 px-6 py-4 text-center text-gray-600">
                    <span v-if="placement.durations.some(d => d.duration_id === duration.id && d.is_confirmed)" class="text-green-600 font-bold">
                      {{ placement.durations.find(d => d.duration_id === duration.id)?.accepted_movie || 'Confirmed' }}
                    </span>
                    <input 
                      v-else 
                      type="checkbox" 
                      :disabled="placement.durations.some(d => d.duration_id === duration.id && d.is_confirmed)"
